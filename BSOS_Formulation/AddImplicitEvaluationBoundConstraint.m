%%%%Program Variables:
 ...
 for i=1:k
    ...
    for j=1:mon_num
        gi=zeros(1,varNum+1);
        gi(dataPointNum*k+mon_num*k+1+(mon_num)*(i-1)+j)=1;% i-th implicit_monomial
        gi(dataPointNum*k+(mon_num)*(i-1)+j)=1;%%i-th curve monomial
        gi(varNum+1)=-1;
        gimplicit=[];
    end
    glk=zeros(1,varNum+1);
    glk(dataPointNum*k+mon_num*k+1+mon_num+k+i)=1;%%lk
    glk(varNum+1)=1;
    pop.G{ineq_count}=[gimplicit;glk];
